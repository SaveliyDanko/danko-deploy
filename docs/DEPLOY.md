...
- **Версия сборки проверяется после запуска:** Ansible записывает commit обновлённого checkout в
  production `.env`, Compose передаёт его backend-контейнеру, `/api/health` возвращает `version`
  из корневого `package.json` и этот commit, а playbook ждёт точного совпадения.
