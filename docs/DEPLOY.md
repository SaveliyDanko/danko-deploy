...
- **Версия сборки проверяется после запуска:** server image получает commit обновлённого checkout,
  `/api/health` возвращает `version` из корневого `package.json` и этот commit, а Ansible ждёт
  точного совпадения перед успешным завершением playbook.
