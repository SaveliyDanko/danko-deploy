...
- **Версия панели:** человекочитаемая версия — поле `version` корневого `package.json`; точную
  сборку определяет Git commit, который Ansible фиксирует в production `.env` и передаёт
  запущенному backend-контейнеру как
  `DANKODEPLOY_COMMIT`. `/api/health` и шапка UI показывают оба значения.
