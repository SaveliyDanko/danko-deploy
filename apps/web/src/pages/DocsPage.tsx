...
const SCRIPT_DEPLOY_AGENT_SPEC = `Подготовь этот проект и его git-репозиторий к деплою скриптом через DankoDeploy (kind: script).

Контекст:
- DankoDeploy раскатывает проект типа "script" так: по SSH в рабочей директории проекта (workdir)
  выполняется "git pull --ff-only", затем "bash ./<deployScript>" (deployScript по умолчанию deploy.sh).
- Опционально есть скрипт снятия (undeployScript), он запускается по кнопке «Снять».
- Скрипт лежит В РЕПОЗИТОРИИ проекта (версионируется вместе с кодом) и запускается на сервере.
- Команды выполняются по SSH БЕЗ TTY (неинтерактивно). Никаких запросов ввода, прогресс-баров с \\r,
  цветных esc-последовательностей быть не должно. Если что-то требует ввода — добавь флаги (-y и т.п.).
- Окружение: панель отдельно умеет писать <workdir>/.env (chmod 600). Скрипт может рассчитывать на
  наличие .env в workdir, но НЕ должен хранить секреты в самом скрипте/репозитории.
- На сервере уже есть то, что нужно проекту (например docker/docker compose, если ты их используешь).
  Скрипт должен в начале ПРОВЕРЯТЬ наличие нужных бинарей и падать с понятным сообщением, если их нет.

Требования к скрипту деплоя (deploy.sh):
1. Начинай с "#!/usr/bin/env bash" и "set -euo pipefail" — любая ошибка обязана прерывать деплой
   с НЕНУЛЕВЫМ кодом возврата (DankoDeploy останавливает деплой по ненулевому коду шага).
2. Будь ИДЕМПОТЕНТНЫМ: повторный запуск на уже раскатанном проекте не должен ломать состояние.
3. Не делай "cd" в абсолютные пути — скрипт уже запускается в workdir. Используй пути относительно
   текущего каталога; если нужен корень репозитория — вычисли его (cd "$(dirname "$0")").
4. Логируй ключевые шаги в stdout короткими строками (echo "==> сборка", "==> миграции", "==> рестарт").
5. В конце сделай smoke-check, что сервис реально поднялся (curl health-эндпоинта / проверка процесса /
   docker compose ps), и упади с ненулевым кодом, если проверка не прошла.
6. Не запускай ничего в фоне без надобности; долгоживущие процессы — через systemd/docker, а не "&".
7. НЕ коммить секреты. Если нужны переменные — читай их из <workdir>/.env (его кладёт панель).

Что нужно сделать с проектом:
1. Определи, как этот проект собирается и запускается (docker compose / сборка артефакта + systemd /
   нативный процесс / миграции БД). Опиши последовательность раскатки одной выкаткой.
2. Создай в корне репозитория исполняемый deploy.sh, реализующий эту последовательность по правилам выше.
   Не забудь "chmod +x deploy.sh" и закоммить файл.
3. Если нужен откат/остановка — создай undeploy.sh (остановить сервис, освободить ресурсы) по тем же правилам.
4. Убедись, что нужные файлы (Dockerfile/compose/конфиги/шаблоны systemd) есть в репозитории и
   путях, на которые ссылается скрипт.
5. Перечисли переменные окружения, которые проект ждёт в <workdir>/.env (имена, без значений).
6. Проверь, что репозиторий клонируется чисто и deploy.sh отрабатывает на свежем checkout.

Формат ответа:

## deploy.sh
\`\`\`bash
<полный итоговый скрипт>
\`\`\`

## undeploy.sh (если нужен)
\`\`\`bash
<полный итоговый скрипт>
\`\`\`
...
- kind: script
- workdir: <путь на сервере>
- deployScript: deploy.sh (или твой путь)
- undeployScript: undeploy.sh (если есть)
- Источник кода: git-public (HTTPS) или git-private (SSH + deploy-ключ)

## Переменные окружения (.env)
- KEY — назначение (без значения)
- ...

## Чек-лист перед деплоем
- В репозитории есть deploy.sh (исполняемый, закоммичен).
- Скрипт неинтерактивен и падает с ненулевым кодом при ошибке.
- Нужные бинари (docker/compose/node/…) есть на сервере или скрипт их проверяет.
- Секретов в скрипте/репозитории нет; чувствительное — в <workdir>/.env.
- Smoke-check в конце скрипта подтверждает, что сервис поднялся.
`;
...
function AgentPromptSection() {
  return (
    <section id="agent-spec" className="scroll-mt-20 space-y-4">
      ...
      <h3 className="pt-2 text-xs font-semibold uppercase tracking-wide text-slate-500">
        Деплой скриптом (kind: script)
      </h3>
      <AgentPromptBlock
        prompt={SCRIPT_DEPLOY_AGENT_SPEC}
        description={
          <>
            Агент подготовит репозиторий к раскатке скриптом: создаст в корне исполняемый{" "}
            <code className="rounded bg-ink px-1">deploy.sh</code> (неинтерактивный, идемпотентный,
            падает по ошибке, со smoke-check в конце) и при необходимости{" "}
            <code className="rounded bg-ink px-1">undeploy.sh</code>. Это то, что DankoDeploy
            запускает после <code className="rounded bg-ink px-1">git pull</code> для проектов типа{" "}
            <code className="rounded bg-ink px-1">script</code>.
          </>
        }
      />
    </section>
