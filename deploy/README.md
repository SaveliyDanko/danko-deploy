...
- Запуск сервера — собранным бандлом `node apps/server/dist/main.js` (esbuild на стадии build
  складывает весь наш TS в один файл; npm-зависимости external). tsx в рантайме НЕ используется —
  это экономит RAM (нет резидентного транспилятора и процесса-обёртки pnpm). Схему БД применяет
  entrypoint (`db:push`, drizzle-kit — миграций в проекте нет).
